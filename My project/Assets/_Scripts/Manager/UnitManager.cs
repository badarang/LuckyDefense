...
public class UnitManager : Singleton<UnitManager>
{
    ...
    private List<Unit>[,] upperUnitGroups = new List<Unit>[Width, Height];
    private List<Unit>[,] lowerUnitGroups = new List<Unit>[Width, Height];
    ...
    private void InitializeUnitGroups()
    {
        for (int x = 0; x < Width; x++)
        {
            ...
            {
                upperUnitGroups[x, y] = new List<Unit>();
                lowerUnitGroups[x, y] = new List<Unit>();
            }
        }
    }
    ...
    public void SummonUnit(bool isMyPlayer = true)
    {
        ...
        List<Unit>[,] unitGroups = isUpper ? upperUnitGroups : lowerUnitGroups;
        ...
        unitGroups[spawnPosition.x, spawnPosition.y].Add(newUnit);
    }
    ...
    private Vector2Int FindEmptyPosition(List<Vector2Int> priorityPositions, List<Unit>[,] unitGroups)
    {
        foreach (var pos in priorityPositions)
        {
            if (unitGroups[pos.x, pos.y].Count == 0)
                return pos;
        }
    ...
    private Vector2Int FindBestMergeablePosition(UnitTypeEnum newUnitType, List<Vector2Int> priorityPositions, List<Unit>[,] unitGroups)
    {
        foreach (var pos in priorityPositions)
        {
            if (unitGroups[pos.x, pos.y].Count > 0 && CanMerge(unitGroups[pos.x, pos.y], newUnitType))
                return pos;
        }
